		...
		<div class="m-portlet">
			<div class="m-portlet__head">
				...
				<div class="m-portlet__head-tools">
					<a href="{{ url('/export/excel/alfitrah/all') }}" class="btn btn-sm btn-success m-btn--air"><i class="fa fa-download"></i> Semua</a>
					<a href="{{ url('/export/pdf/alfitrah/all') }}" class="btn btn-sm btn-danger m-btn--air"><i class="fa fa-download"></i> Semua</a>
				</div>
			</div>
			<div class="m-portlet__body">
				@include('interface.backend.plugin.date_filter')
				<form class="form" action="{{ url('export/pdf/alfitrah') }}" method="GET" id="pdf">
					{{ csrf_field() }}
					<input type="hidden" name="start" class="p_start">
					<input type="hidden" name="end" class="p_end">
				</form>
				<form class="form" action="{{ url('export/excel/alfitrah') }}" method="GET" id="excel">
					{{ csrf_field() }}
					<input type="hidden" name="start" class="p_start">
					<input type="hidden" name="end" class="p_end">
				</form>
...
$(document).ready(function(){
	...
	var table = $('#m-datatable').DataTable({
			...
			processing: true,
            ajax: "{{ url('/admin/alokasi/listfitrah') }}",
            columns: [
            			{
