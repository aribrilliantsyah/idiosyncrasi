		...
		<div class="m-portlet">
			<div class="m-portlet__head">
				...
				<div class="m-portlet__head-tools">
					<a href="{{ url('/export/excel/cmal/all') }}" class="btn btn-sm btn-success m-btn--air"><i class="fa fa-download"></i> Semua</a>
					<a href="{{ url('/export/pdf/cmal/all') }}" class="btn btn-sm btn-danger m-btn--air"><i class="fa fa-download"></i> Semua</a>
				</div>
			</div>
			<div class="m-portlet__body">
				@include('interface.backend.plugin.date_filter')
				<form class="form" action="{{ url('export/pdf/cmal') }}" method="GET" id="pdf">
					{{ csrf_field() }}
					<input type="hidden" name="start" class="p_start">
					<input type="hidden" name="end" class="p_end">
				</form>
				<form class="form" action="{{ url('export/excel/cmal') }}" method="GET" id="excel">
					{{ csrf_field() }}
					<input type="hidden" name="start" class="p_start">
					<input type="hidden" name="end" class="p_end">
				</form>
   ...
   $.fn.dataTable.ext.search.push(
	...
	$(document).ready(function(){
		...
		var table = $('#m-datatable').DataTable({
				...
				processing: true,
                ajax: "{{ url('/admin/perolehan/listmal') }}",
                columns: [
                			{
