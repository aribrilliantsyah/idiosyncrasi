...
@include('backend.master.kategori_post.form')
@endsection
@push('plugin-scripts')
...
<script type="text/javascript">
	...
	function editForm(id) {
		...
		$.ajax({
			url: "{{ url('/admin/kategori_post') }}" + '/' + id + "/edit",
			type: "GET",
			dataType: "JSON",
	...
	$(document).ready(function(){
        ...
        $('#f_kategori input').each(function(){ 
         ...
         $('#f_kategori').submit(function(e){
            ...
            //url
            var id = $('#id').val();
            if (save_method == 'add') url = "{{ url('/admin/master/kategori_post/') }}";
            else url = "{{ url('/admin/master/kategori_post/'). '/'}}" + id;
	...
	$('#delsel').click(function(){
		...
		if(id.length === 0) {
			...
			swal({
				...
				$.ajax({
					url:'{{ url('/admin/master/kategori_post/delsel') }}',
					method:'POST',
					data:{
						'_token': '{{ csrf_token() }}',
						'id':id
